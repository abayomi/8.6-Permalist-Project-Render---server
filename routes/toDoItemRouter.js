...
toDoItemsWinstonLogger.info(
  "Info log: setting up program to read env file, in toDoItemRouter."
);
env.config();
toDoItemsWinstonLogger.info(
  "Info log: new pg client creating, in toDoItemRouter."
);
...
pool.on("error", (err) => {
  toDoItemsWinstonLogger.error(
    "error log: something has gone wrong with db connection, in toDoItemRouter!",
    err.stack
  );
});

toDoItemsWinstonLogger.info("Info log: middleware starting, in toDoItemRouter");
app.use(express.urlencoded({ extended: true }));
app.use(express.json());
toDoItemsWinstonLogger.info("Info log: midleware ending, in toDoItemRouter");
...
async function performQueryReturnResult(queryStatement) {
  ...
  return new Promise((resolve, reject) => {
    toDoItemsWinstonLogger.info(
      "Info log: Creating database connection, in performQueryReturnResult, in toDoItemRouter."
    );
    let result;
    const y = pool.connect(async (err, client, release) => {
      if (err) {
        toDoItemsWinstonLogger.error(
          "Error log: Creating database connection crashed, so exiting, in performQueryReturnResult, in toDoItemRouter."
        );
        emitter.emit("queryCompleted");
        resolve([undefined, undefined]);
      }
      toDoItemsWinstonLogger.info(
        "Info log: query sending to DB, in performQueryReturnResult, in toDoItemRouter."
      );
      client.query(queryStatement, (err, res) => {
        toDoItemsWinstonLogger.info(
          "Info log: releasing connection back to pool, in performQueryReturnResult, in toDoItemRouter."
        );
        ...
        if (err) {
          toDoItemsWinstonLogger.error(
            "Error log: Error running query on db, in performQueryReturnResult, in toDoItemRouter."
          );
          resolve([undefined, undefined]);
        }
        toDoItemsWinstonLogger.info(
          "Info log: Results obtained from query on db, in performQueryReturnResult, in toDoItemRouter."
        );
...
async function getItems() {
  toDoItemsWinstonLogger.info(
    "Info log: Starting getItems, in toDoItemRouter."
  );
  let response = await performQueryReturnResult("select * from items");
  toDoItemsWinstonLogger.info("Info log: Ending getItems, in toDoItemRouter.");
  return response;
}
...
toDoItemRouter.get(
  ...
  tryCatchAsyncController(async (req, res, next) => {
    toDoItemsWinstonLogger.info(
      "Info log: starting fetching items, in toDoItemRouter."
    );
    ...
    if (response[1] == undefined) {
      toDoItemsWinstonLogger.info(
        "Info log: There were no errors while fetching items, in toDoItemRouter."
      );
      res.send({ listTitle: "ARC To Do List", listItems: response[0] });
    } else res.send({ error: "Fetching items failed." });
    toDoItemsWinstonLogger.info(
      "Info log: ending fetching items in, in toDoItemRouter."
    );
  })
);
...
toDoItemRouter.post(
  ...
  tryCatchAsyncController(async (req, res, next) => {
    toDoItemsWinstonLogger.info(
      "Info log: starting adding an item , in toDoItemRouter"
    );
    ...
    if (item != undefined) {
      ...
      toDoItemsWinstonLogger.info(
        "Info log: ending adding an item, in toDoItemRouter"
      );
    } else {
      res.send({ error: "Your last operation encountered an error." });
      toDoItemsWinstonLogger.error(
        "Error log: there was no item sent to the server but the add method was called, in toDoItemRouter."
      );
    }
  })
);
...
toDoItemRouter.patch(
  ...
  tryCatchAsyncController(async (req, res, next) => {
    ...
    if (updatedItemId != undefined && updatedItemTitle != undefined) {
      ...
      res.send({ message: "Success" });
      toDoItemsWinstonLogger.info(
        "Info log: ending editing an item, in toDoItemRouter"
      );
    } else {
      res.send({ error: "Your last operation encountered an error." });
      toDoItemsWinstonLogger.error(
        "Error log: either the item's ID or body were missing, but the edit method was called, in toDoItemRouter."
      );
    }
  })
);
